...
// 👇 NUEVO: Doble memoria para Retargeting Inteligente 👇
const listaUsuariosBot = new Set(); // Todos los que han entrado
const usuariosIniciados = new Set(); // Solo los que tocaron "Ir a la Web"
...
bot.onText(/\/start/, (msg) => {
    ...
    if (chatId.toString() === ADMIN_ID) {
        ...
        // 👇 BOTÓN NUEVO PARA DESPERTAR INACTIVOS 👇
...
bot.onText(/\/masivo (.+)/, (msg, match) => {
    ...
    bot.sendMessage(ADMIN_ID, `⏳ Iniciando envío masivo...`);
...
bot.on('callback_query', (query) => {
    ...
    if (query.data.startsWith('masivo_') && chatId.toString() === ADMIN_ID) {
        ...
        if (query.data === 'masivo_cupos') {
            ...
            esParaInactivos = true;
            mensajeMasivo = "👀 *¡Aún guardamos tu cupo!*\nVi que iniciaste el bot pero no entraste a la plataforma. Quedan muy pocos lugares.\n\n👉 Toca 'Iniciar Método' en el menú principal ahora mismo para registrarte y asegurar tus recompensas.";
        }

        bot.sendMessage(ADMIN_ID, `⏳ Enviando anuncio...`);
        
        listaUsuariosBot.forEach(usuarioId => {
            // Lógica inteligente: Si es para inactivos, solo enviar si no están en 'usuariosIniciados'
            if (esParaInactivos) {
                if (!usuariosIniciados.has(usuarioId) || usuarioId.toString() === ADMIN_ID) {
                    bot.sendMessage(usuarioId, `📢 *ANUNCIO:* \n\n${mensajeMasivo}`, { parse_mode: 'Markdown' }).catch(e=>{});
                }
            } else {
                // Si es un aviso general, enviar a todos
                bot.sendMessage(usuarioId, `📢 *ANUNCIO:* \n\n${mensajeMasivo}`, { parse_mode: 'Markdown' }).catch(e=>{});
            }
        });
    ...
    // REGISTRAMOS QUE YA INICIÓ EL MÉTODO Y NO ES INACTIVO
    ...
    if (query.data === 'requisitos') {
        if (chatId.toString() !== ADMIN_ID) { bot.sendMessage(ADMIN_ID, `🖱️ *NUEVO CLIC*\n👤 Nombre: ${firstName}\n🔗 Usuario: ${username}\n👆 Acción: Vio los requisitos`).catch(e=>{}); }
    ...
    if (query.data === 'info_referidos') {
        if (chatId.toString() !== ADMIN_ID) { bot.sendMessage(ADMIN_ID, `🖱️ *NUEVO CLIC*\n👤 Nombre: ${firstName}\n🔗 Usuario: ${username}\n👆 Acción: Vio info Referidos`).catch(e=>{}); }
...
app.listen(PORT, () => { console.log(`🌐 Servidor web fantasma encendido.`); });
